...
class NoticeEconomy: NoticeBaseModel {
    static func parseListEconomics(page: Int, keyword: String?, completion: @escaping ([Notice]) -> Void) {
        ...
        print("[DDD] requestURL : \(requestURL)")
        self.cleanList()
            ...
            case .success(_):
                if let data = response.value {
                    do {
                        ...
                        for product in doc.css("div[class='notice_list']") {
...
//                                    }
                                case 1: // 제목 (첨부파일 유무 포함)
                                    ...
                                    hasAttachment = false
                                    for _ in td.css("i[class='fa fa-download']") {
                                        hasAttachment = true
                                    }
